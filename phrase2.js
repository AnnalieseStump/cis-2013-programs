...
if(stringEntry==="")//checking if a value was entered
{
...
var consonants = function () 
{
    ...
    else 
    {
        ...
        for(var intCount=0; intCount<stringEntry.length; intCount++)
        {
            switch(stringEntry.charAt(intCount))//go through each character of that entry with our counter
            {
                case "a":
                case "e":
                case "i":
                case "o":
                case "u":
                    break; //if a vowel, break out of it each time
                default:
                    intConsonants++; //if not a value, continue on to the next variable
                    break;
            }
        }
