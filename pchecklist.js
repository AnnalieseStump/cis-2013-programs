...
{
    var stringPassword, arrayBadPasswords, stringOutput; //declaring variables
    var a; //declaring counter
    
    stringPassword = $("password").value; //calling on the imput
    ...
    stringOutput = "";
            ...
            for (a=0; a<arrayBadPasswords.length; a++) //setting up the array to be accessed
            {
                if (stringPassword==arrayBadPasswords[a] || stringPassword.length < 8)//if the imput is in the array or less than 8 characters, it goes through this loop
                {
                    stringOutput="Sorry, your password is not strong. Try another one!";
                    break; //stopping the loop when the imput is invalid
                }
                else 
                {
                    stringOutput="That is likely a strong password! It should be safe to use :)";//when the imput is both 8 or more characters and not in the array list
                }
            }
        
$("output").value = stringOutput; //sending the output back
};

window.onload = function () 
{
   $("password").focus();
   $("check_password").onclick = passwordCheckOld; //activating the button to show the output when clicked
};